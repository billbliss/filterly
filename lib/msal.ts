import { ConfidentialClientApplication, PublicClientApplication, LogLevel, type DeviceCodeRequest } from '@azure/msal-node';
import { kvGet, kvSet } from './kv';

const getAuthority = () => `https://login.microsoftonline.com/${process.env.TENANT_ID || 'common'}`;
...
export const scopes = ['openid','profile','offline_access','User.Read','Mail.ReadWrite'];

const appOnlyScopes = ['https://graph.microsoft.com/.default'];
const AUTH_MODE = process.env.AUTH_MODE; // 'app' | 'device' | undefined
  ...
  cache: {
    cachePlugin: {
      ...
      afterCacheAccess: async (ctx) => {
        if (ctx.cacheHasChanged) await kvSet(MSAL_CACHE, ctx.tokenCache.serialize());
      }
    }
  }
});
...
const pca = new PublicClientApplication({
  ...
  cache: {
    cachePlugin: {
      ...
      afterCacheAccess: async (ctx) => {
        if (ctx.cacheHasChanged) await kvSet(MSAL_CACHE + ':pca', ctx.tokenCache.serialize());
      }
    }
  }
});
...
async function getAppOnlyToken() {
  const result = await msalApp.acquireTokenByClientCredential({ scopes: appOnlyScopes });
...
export async function acquireTokenByCode(code: string) {
  return msalApp.acquireTokenByCode({ code, scopes, redirectUri: process.env.REDIRECT_URI! });
}
...
export async function getAccessToken() {
  {
    ...
    if (AUTH_MODE === 'device') {
      ...
      if (pcaAccounts.length) {
        const res = await pca.acquireTokenSilent({ account: pcaAccounts[0], scopes });
    ...
    if (AUTH_MODE === 'app') {
      if (!process.env.CLIENT_SECRET) throw new Error('AUTH_MODE=app but CLIENT_SECRET is not set');
    ...
    if (ccaAccounts.length) {
      const res = await msalApp.acquireTokenSilent({ account: ccaAccounts[0], scopes });
      return res!.accessToken;
    }
